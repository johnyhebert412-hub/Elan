(function () {
  ...
  const suggestedActions = {
    ...
    budget: { title: "Vérifier ton solde", text: "Ouvre ton compte et regarde le montant disponible.", action: "complete" },
  ...
  const homeDomains = {
    ...
    budget: { title: "Budget", subtitle: "Voir ce qui s'en vient. Décider posément.", missions: [{ label: "Vérifier le solde" }, { label: "Attendre avant un achat" }, { label: "Payer une facture" }, { label: "Noter une dépense" }, { label: "Annuler un abonnement inutile" }, { label: "Mettre un petit montant de côté" }, { label: "Lister les dépenses de la semaine" }, { label: "Vérifier les abonnements actifs" }, { label: "Comparer deux prix avant d'acheter" }, { label: "Faire une liste avant d'aller au magasin" }, { label: "Revoir un poste de dépense" }, { label: "Éviter une dépense impulsive" }, { label: "Vérifier une date de paiement" }, { label: "Planifier l'épicerie de la semaine" }] },
  ...
  const BUDGET_ACTIONS = [
    ...
    "Vérifier une date de paiement à venir",
    "Faire une liste d'épicerie avant d'acheter",
    "Éviter un achat impulsif aujourd'hui",
    "Vérifier mes abonnements actifs",
    "Comparer deux prix avant de décider",
    "Mettre un montant de côté pour l'impôt",
    "Planifier un paiement de carte de crédit"
  ];
  ...
  function renderBudget() {
    ...
    if (availableEl) {
      ...
      availableEl.className = "budget-available-value" +
        (available !== null && available < 0 ? " budget-available-negative" : available !== null && available >= 0 ? " budget-available-positive" : "");
    }
    // Message contextuel selon le disponible
    const ctxMsg = $("budget-context-msg");
    if (ctxMsg) {
      if (available === null) {
        ctxMsg.textContent = "Ajoute tes revenus et obligations pour voir ton disponible.";
        ctxMsg.className = "budget-context-msg budget-ctx-neutral";
      } else if (available < 0) {
        ctxMsg.textContent = "Attention — tes obligations dépassent tes revenus. Prends un moment pour revoir ça.";
        ctxMsg.className = "budget-context-msg budget-ctx-alert";
      } else if (available < 100) {
        ctxMsg.textContent = "Marge serrée. Évite les achats non planifiés pour l'instant.";
        ctxMsg.className = "budget-context-msg budget-ctx-caution";
      } else {
        ctxMsg.textContent = "Tu as de la marge. Reste attentif aux dépenses impulsives.";
        ctxMsg.className = "budget-context-msg budget-ctx-ok";
      }
    }
    ...
    if (billsList) {
      ...
      if (sorted.length === 0) {
        billsList.innerHTML = "<li class=\"budget-bills-empty\">Aucun paiement ajouté. Ajoute ton loyer, ta carte ou ton véhicule pour garder un œil dessus.</li>";
      } else {
        ...
        billsList.replaceChildren(...sorted.slice(0, 8).map((bill) => {
          ...
          li.innerHTML = `
            ...
            <div class="budget-bill-info">
              <span class="budget-bill-name">${icon} ${bill.name}</span>
